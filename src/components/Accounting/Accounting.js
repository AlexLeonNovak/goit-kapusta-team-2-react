...
export const Accounting = ({type, onChangeCategory}) => {
	...
	const amounts = transactions.reduce((acc, transaction) => {
		return {
			...acc,
			[transaction.category._id]: (acc[transaction.category._id] || 0) + transaction.amount
		}
	}, {});
		...
		<div className={s.categoriesListContainer}>
			<ul className={s.categoriesList}>
				{categories.map((category, index) => {
					...
					return (
					<li key={category._id}
					    className={classNames(s.categoryItem, {[s.active]: currentIndex === index})}
					    onClick={() => setCurrentIndex(index)}
					>
						<span className={s.container_item_txt}>{amounts[category._id] || 0}</span>
						<div className={s.categoryLogoWrapper}>
							{isSvg ? (
								<SVG className={s.categoryLogo}
								     width={65}
								     height={57}
								     title={category.name}
